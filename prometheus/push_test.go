...
func TestPush(t *testing.T) {
	...
	metric1 := NewCounter(CounterOpts{
		Name: "testname1",
		Help: "testhelp1",
	})
	metric2 := NewGauge(GaugeOpts{
		Name:        "testname2",
		Help:        "testhelp2",
	...
	reg := NewRegistry()
	MustRegisterWith(reg, metric1)
	MustRegisterWith(reg, metric2)
	...
	if err := PushCollectors("testjob", GroupByHostname(), pgwOK.URL, metric1, metric2); err != nil {
		t.Fatal(err)
	}
	if lastMethod != "PUT" {
	...
	if err := PushAddCollectors("testjob", nil, pgwOK.URL, metric1, metric2); err != nil {
		t.Fatal(err)
	}
	if lastMethod != "POST" {
	...
	if err := PushCollectors("testjob", nil, pgwErr.URL, metric1, metric2); err == nil {
	...
	if err := PushCollectors("testjob", map[string]string{"foo": "bums"}, pgwErr.URL, metric1, metric2); err == nil {
		t.Error("push with grouping contained in metrics succeeded")
	}
	if err := PushCollectors("test/job", nil, pgwErr.URL, metric1, metric2); err == nil {
		t.Error("push with invalid job value succeeded")
	}
	if err := PushCollectors("testjob", map[string]string{"foo/bar": "bums"}, pgwErr.URL, metric1, metric2); err == nil {
	...
	if err := Push(reg, "testjob", GroupByHostname(), pgwOK.URL); err != nil {
		t.Fatal(err)
	}
	if lastMethod != "PUT" {
	...
	if err := PushAdd(reg, "testjob", map[string]string{"a": "x", "b": "y"}, pgwOK.URL); err != nil {
		t.Fatal(err)
	}
	if lastMethod != "POST" {
