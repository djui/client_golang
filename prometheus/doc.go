...
// the registry.
//
// A particular error case to handle is if a Collector has already be registered
// (or a different Collector that collects the same metrics). Registration fails
// in that case, but you can detect from the kind of error what has
// happened. Sometimes you want to register the new Collector if an equal
// Collector hasn't been registered before, but otherwise you want to switch
// over to the previously registered Collector. This pattern is supported by the
// RegisterOrGet function.
//
...
// RegisterWithOrGet, MustRegisterOrGet → MustRegisterWithOrGet.
//
// There are a number of uses for custom registries: You can create registries
// with special properties, see NewPedanticRegistry or
// NewRegistryWithInjectionHook as examples. You can avoid global state, as it
// is imposed by the DefaultRegistry. You can use multiple registries at the
...
//
// The functions Push and PushAdd push the metrics from a custom
// registry. However, often you just want to push a handful of Collectors only,
// without going through registration first. For that case, there are the
// convenience functions PushCollectors and PushAddCollectors. See the
// documentation of the Pushgateway to understand the differences between Push
// and PushAdd and the meaning of the grouping parameters:
