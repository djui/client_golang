...
// Push triggers a metric collection by the provided registry and pushes all
...
func Push(r Registry, job string, grouping map[string]string, url string) error {
	return push(r, job, grouping, url, "PUT")
}

// PushAdd works like Push, but only previously pushed metrics with the same
// name (and the same job and other grouping labels) will be replaced. (It uses
// HTTP method 'POST' to push to the Pushgateway.)
func PushAdd(r Registry, job string, grouping map[string]string, url string) error {
	return push(r, job, grouping, url, "POST")
}

func push(r Registry, job string, grouping map[string]string, pushURL, method string) error {
...
// PushCollectors works like Push, but it does not collect via a
// registry. Instead, it collects from the provided collectors directly. It is a
// convenient way to push only a few metrics.
func PushCollectors(job string, grouping map[string]string, url string, collectors ...Collector) error {
	return pushCollectors(job, grouping, url, "PUT", collectors...)
}

// PushAddCollectors works like PushAdd, but it does not collect via a
// registry. Instead, it collects from the provided collectors directly. It is a
// convenient way to push only a few metrics.
func PushAddCollectors(job string, grouping map[string]string, url string, collectors ...Collector) error {
	return pushCollectors(job, grouping, url, "POST", collectors...)
}

func pushCollectors(job string, grouping map[string]string, url, method string, collectors ...Collector) error {
	r := NewRegistry()
...
// GroupByHostname returns a label map with the only entry
