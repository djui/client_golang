...
// NewRegistryWithInjectionHook creates a registry with the provided hook to inject
// MetricFamilies. The hook is a function that is called whenever metrics are
// collected. The MetricFamily protobufs returned by the hook function are
// merged with the metrics collected in the usual way.
//
...
// RegisterWithOrGet registers the provided Collector with the provided Registry
// but does not return an error if a Collector is registered that equals a
// previously registered Collector. (Two Collectors are considered equal if
// their Describe method yields the same set of descriptors.) Instead, the
// previously registered Collector is returned (which is helpful if the new and
// previously registered Collectors are equal but not identical, i.e. not
// pointers to the same object).
//
// As for RegisterWith, it is still not safe to call RegisteWithrOrGet with the
// same Collector multiple times concurrently.
func RegisterWithOrGet(r Registry, c Collector) (Collector, error) {
	if err := r.Register(c); err != nil {
...
// MustRegisterWithOrGet works like RegisterWithOrGet but panics where RegisterWithOrGet
// would have returned an error.
func MustRegisterWithOrGet(r Registry, c Collector) Collector {
	existing, err := RegisterWithOrGet(r, c)
	if err != nil {
		panic(err)
	}
	return existing
...
//
// RegisterOrGet is a shortcut for RegisterWithOrGet(DefaultRegistry, c). See
// there for more details.
func RegisterOrGet(c Collector) (Collector, error) {
	return RegisterWithOrGet(DefaultRegistry, c)
}

// MustRegisterOrGet behaves like RegisterOrGet but panics instead of returning
// an error.
//
// MustRegisterOrGet is a shortcut for MustRegisterWithOrGet(DefaultRegistry,
// c). See there for more details.
func MustRegisterOrGet(c Collector) Collector {
	return MustRegisterWithOrGet(DefaultRegistry, c)
}
...
// AlreadyRegisteredError is returned by the "RegisterOrGet" type of
// registration calls. It contains the existing Collector and the (rejected) new
// Collector that equals the existing one.
